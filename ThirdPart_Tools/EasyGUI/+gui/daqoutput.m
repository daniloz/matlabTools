...
%TODO: Remove ValueChangedFcn... possible ???
%TODO: Include DeviceName in properties
...
classdef (Sealed) daqoutput < gui.widget
    ...
    properties (Dependent, GetAccess=public, SetAccess=private)
        ...
        %    true  => device is already initialized
        %    false => device is not yet initialized
        %
        Ready
    ...
    properties (SetObservable)
        ...
        % ConfigChangedFcn - Function to be called whenever the Configuration is updated.
        % 
        %   ConfigChangedFcn specifies an optional function handle. Whenever
        %   the Configuration is updated, this function will be invoked
        %   with one input argument (the handle to the widget). 
        %   To disable ConfigChangedFcn, set it to [].
        % 
        ConfigChangedFcn = []   
    end
    ...
    methods
        ...
        function delete(obj)       
            ...
            delete(obj.AoObj);
            if ~isempty(obj.ConfigChangedFcnListener) && isvalid(obj.ConfigChangedFcnListener)
                delete(obj.ConfigChangedFcnListener);
            end
        end
    ...
    methods
        ...
        function set.Value(obj, val)
            [~, nCH] = size(val);
            if nCH ~= length(obj.Channels),
                throw(MException('daqoutput:InvalidData', ...
                    'Value must have a column of data for each channel.'));
            end
            if isempty(obj.AoObj) || ~isvalid(obj.AoObj)
                try
        ...
        function set.ConfigChangedFcn(obj, f)
            if isa(f, 'function_handle')
                obj.ConfigChangedFcn = f;
                if isempty(obj.ConfigChangedFcnListener) 
                    obj.ConfigChangedFcnListener = event.listener(obj, 'ConfigChanged', @(src,eventdata) f(src));
                    % listener cannot trigger a ConfigChanged event
                    obj.ConfigChangedFcnListener.Recursive = false;
                else 
                    obj.ConfigChangedFcnListener.Callback = @(src,eventdata) f(src);
                end
            elseif isempty(f)
                obj.ConfigChangedFcn = f;
                if ~isempty(obj.ConfigChangedFcnListener)
                ...
                obj.ConfigChangedFcnListener = [];
            else
                throw(MException('gui.daqoutput:InvalidConfigChangedFcn', 'ConfigChangedFcn should be a function handle'));
            end
        end
        ...
        function start(obj)
            ...
            if isempty(obj.Value)
                throw(MException('daqoutput:NoData', ...
                    'Load some data before starting the device.'));                
            end
            if isempty(obj.AoObj) || ~isvalid(obj.AoObj)
                try
                    initializeAnalogOutput(obj);
