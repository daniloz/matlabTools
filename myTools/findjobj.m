...
% $Revision: 1.19 $  $Date: 2009/04/23 22:17:09 $
...
%% Present the object hierarchy tree
    function presentObjectTree()
        ...
        setTreeNodeIcon(root,container);  % constructor must accept a char unfortunately, so need to do this afterwards...
...
%% Debuggable "quiet" error-handling
    function dispError
        ...
        for idx = 1 : length(err.stack)
            ...
            if ~isempty(regexpi(filename,'findjobj'))
