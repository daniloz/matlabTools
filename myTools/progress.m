...
if exist('title','var')
	...
	if cont % If not in init stage, give a void title
		title = '';
		back  = ''; % has to be set
	else    % else set a default title CENTERED
		title_def = 'Please wait...';
        Ltitle = length(title_def);
        title = [rep(' ',floor((lmax-Ltitle)/2)) title_def rep(' ',ceil((lmax-Ltitle)/2)) '\n'];
	end
end
